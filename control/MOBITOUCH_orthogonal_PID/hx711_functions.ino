...
void setupHX711Calibration() {
  ...
  Serial.println("\nStep 2: Scale calibration");
  Serial.println("Place a known weight (e.g., 100g, 500g) on the load cell");
  Serial.println("After placing the weight, press any key and Enter");
}
...
void loopHX711Calibration() {
  ...
  if (!calibration_done && waiting_for_input) {
    ...
    if (Serial.available()) {
      ...
      Serial.println("\nEnter the placed weight in grams (e.g., 100):");
